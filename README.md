...
Проект реализован как multi-module Maven project и состоит из двух независимых Spring Boot приложений:

- `backend` — REST API для работы с клиентами, адресами, поиском и валидацией данных.
- `frontend` — веб-интерфейс на Thymeleaf, который обращается к backend через WebClient.
...
Backend предоставляет REST API для работы с клиентами и сетевыми адресами. Также содержатся unit-тесты для сервисного слоя.
...
Backend выполняет основную валидацию входных данных. Проверяются:
- обязательность полей;
- длина строковых значений;
- допустимые символы в имени клиента;
- формат IPv4;
- формат MAC-адреса;
- допустимые значения типа клиента.

### Frontend

Frontend предоставляет веб-интерфейс для работы с данными.
UI-приложение не обращается к базе данных напрямую. Все операции выполняются через HTTP-запросы к backend-приложению с использованием `WebClient`.
